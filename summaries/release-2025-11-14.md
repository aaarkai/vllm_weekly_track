# Weekly Release Report for vllm-project/vllm (2025-11-14)

This week merged 226 PRs from 128 contributors. Key areas: features 7, fixes 18, performance 29.

## Executive Summary

本周vLLM发布重点包括功能增强、错误修复、性能优化及模型支持更新。功能方面，OpenAI API服务器新增SageMaker标准动态LoRA适配器和状态会话管理装饰器，并完成编码器-预填充-解码解耦的编码器分离工作。错误修复涉及Apple Silicon M1编译、FA注意力下沉支持、多专家模型配置及多模态解析等关键问题。

性能优化聚焦内核融合，如将Qwen模型的QK Norm与RoPE合并为单一CUDA内核，实现Allreduce融合并调整阈值，同时改进CUDA图支持与内存管理。模型与硬件支持修复了Qwen2.5-Omni视频处理参数，优化AMD MI308X的Triton MoE配置，并启用XPU专家并行。破坏性变更包括移除未实现方法警告及重写C++元函数至Python，升级时需验证环境兼容性。
...
* [CI] Reduce Blackwell Fusion test runtime by filtering tests and only run all tests in nightly ([#28074](https://github.com/vllm-project/vllm/pull/28074)) by @copilot-swe-agent
