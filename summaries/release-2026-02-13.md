# Weekly Release Report for vllm-project/vllm (2026-02-13)

This week merged 245 PRs from 133 contributors. Key areas: features 8, fixes 65, performance 28.

## Executive Summary

本周版本在核心注意力机制、硬件支持与模型优化方面进行了重要更新。主要亮点包括新增 FlashInfer Sparse MLA 后端支持，以及对 ROCm 平台上 GPT_OSS (amd-quark 格式) 模型加载的增强。功能方面，扩展了对 Nemotron (TP4, B200)、ModelOpt MXFP8 密集模型以及 MiniCPM-o 等模型和硬件的支持，并增强了 CPU 绑定与嵌入输入功能。

错误修复是本次更新的重点，涉及范围广泛，包括修复了 Mamba、MoE、DeepSeek-V3.2、Qwen3.5 等多个模型在缓存、精度、张量验证等方面的问题，同时解决了 ROCm、CPU 等平台及 Whisper、FunASR 等组件的多项缺陷。性能优化工作包括启用默认的 torch.compile 融合以提升效率，并改进了基准测试工具。代码重构方面，引入了 MoEActivation 枚举类型并合并了序列处理逻辑，建议开发者关注相关 API 的变更。
...
- Note: The use of `PromptType | DictPrompt | TokPrompt` as parameter types is temporary and can be simplified to just `TokPrompt` once all APIs use Renderer.
