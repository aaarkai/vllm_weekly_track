# Weekly Release Report for vllm-project/vllm (2026-01-23)

This week merged 180 PRs from 97 contributors. Key areas: features 6, fixes 28, performance 16.

## Executive Summary

本周发布聚焦于混合专家（MoE）架构的持续重构与低精度计算优化。核心工作包括为 MoE 路由器和 FP8 线性运算引入新的抽象层与内核选择策略，旨在提升系统模块化与计算效率。此外，新增了对 GLM-4.7 及 Nemotron 系列视觉语言模型的支持，并扩展了多项功能的硬件兼容性。

性能优化方面，针对 CPU 后端、注意力内核及推测解码进行了改进。本周修复了大量问题，涉及 DeepSeek、FlashInfer、Eagle 推测解码、多个视觉模型及 ROCm 平台等多个关键领域，显著提升了系统稳定性。需要注意的变更是移除了多项已弃用的环境变量，升级时需检查相关配置。
...
- Note: Skipped Idefics3 as the tower and language model are contained in the same child module. This issue can technically be solved by reorganizing the modules and updating the weight mapping accordingly but it will make things more complicated. We can revisit this model later if the need arises.
- Note: Skipped GLM4V (old version) as the tower and language model are contained in the same child module. This issue can technically be solved by reorganizing the modules and updating the weight mapping accordingly but it will make things more complicated. We can revisit this model later if the need
- Note: Skipped Chameleon as it is a monolithic model without separate tower and language components.
- Verify dependency resolution and tested code paths that use the upgraded Starlette/FastAPI dependencies:

## Contributors

@Alnusjaponica, @AndreasKaratzas, @AuYang261, @Chenyaaang, @DanielMe, @DarkLight1337, @DemingCheng, @George-Polya, @Harry-Chen, @Isotr0py, @JJJYmmm, @Jackmin801, @LucasWilkinson, @Lucaskabela, @MatthewBonanni, @NickLucche, @ProExpertProg, @RishabhSaini, @T1mn, @TomerBN-Nvidia, @Wauplin, @WoosukKwon, @alex-jw-brooks, @alexsun07, @amd-hhashemi, @amirkl94, @bnellnm, @carlory, @chaunceyjiang, @cjackal, @danisereb, @divakar-amd, @diviramon, @dolpm, @eldarkurtic, @elvischenv, @fadara01, @ganyi1996ppo, @gcanlin, @gmagogsfm, @gopalsarda, @halyavin, @honglyua-il, @huydhn, @hyeongyun0916, @iboiko-habana, @ikaadil, @ilmarkov, @jasonyanwenl, @jeejeelee, @jiahanc, @karanb192, @kebe7jun, @knlnguyen1802, @lingebeng, @liranschour, @longregen, @ltd0924, @mawong-amd, @maxdebayser, @mgoin, @micah-wil, @mritunjaysharma394, @netanel-haber, @njhill, @noooop, @orozery, @pacoxu, @patrickvonplaten, @paulpak58, @qandrew, @qli88, @rabi, @rahul-tuli, @randzero, @rasmith, @ricky-chaoju, @robertgshaw2-redhat, @shanjiaz, @simon-mo, @tjtanaa, @tlrmchlsmth, @tom-zju, @tomasruizt, @vadiklyutiy, @vanbasten23, @vanshilshah97, @vkuzo, @vllmellm, @walterbm, @whx-sjtu, @xhx1022, @xyang16, @yewentao256, @ys950902, @zRzRzRzRzRzRzR, @zou3519