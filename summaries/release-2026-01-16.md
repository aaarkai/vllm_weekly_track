# Weekly Release Report for vllm-project/vllm (2026-01-16)

This week merged 231 PRs from 117 contributors. Key areas: features 5, fixes 22, performance 16.

## Executive Summary

本次发布的核心更新包括对多模态模型的支持与代码重构。主要新增了Molmo2多模态模型支持，并重组了多模态处理及注意力相关代码以优化架构。功能上引入了K-EXAONE-236B-A23B等新模型，同时为Marlin MoE添加了thread_n=64支持，并扩展了OpenAI API对skip_special_tokens的配置。

缺陷修复集中解决了一系列关键问题，包括ROCm Dockerfiles构建、DeepSeek模型在ROCm上的精度异常、CUDA Graph在Model Runner V2中的错误，以及混合负载均衡和推测解码中的元数据一致性。性能方面，针对MoE内核进行了多项优化，并增强了基准测试工具的功能。

模型与硬件支持进一步扩展，新增Step3vl 10b模型，为压缩张量模型引入了MXFP4 W4A16量化支持，并修复了多项ROCm平台及特定模型（如Qwen3-VL-Reranker、Mamba）的兼容性问题。本次更新包含部分破坏性变更，涉及缓存工厂迁移、冗余代码清理及工具函数重构，建议升级时注意相关API变动。
...
- NOTE: We still need to fix FlashInfer attention metadata builder has race conditions
