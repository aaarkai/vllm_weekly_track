# Weekly Release Report for vllm-project/vllm (2026-01-09)

This week merged 212 PRs from 111 contributors. Key areas: features 5, fixes 10, performance 16.

## Executive Summary

本周发布围绕 MoE 架构重构、多模态模型支持扩展及内核性能优化展开核心工作。MoE 方面，对 Fp8 和 NVFP4 数据类型的重构持续进行，并修复了相关内核问题。功能增强包括为 DeepSeek-OCR 等模型添加 LoRA 支持，并扩展了 Triton 注意力后端对编码器场景的支持。性能提升显著，通过对 NVFP4 MoE 内核的融合与优化，实现了端到端吞吐与首字延迟的改善。

模型支持范围进一步扩大，新增了对 Grok-2、LFM2-VL 等模型的支持，并为 GLM-4-V、LLaVA 等多个视觉语言模型的塔式结构与连接器启用了 LoRA。修复了 GLM 系列模型在特定配置下的工具调用与思维链问题。本次发布包含数项突破性变更，主要涉及废弃部分 Attention 元数据属性的使用、强化请求参数验证，以及对代码库的清理与重构，用户在升级时需留意相关变更。
...
- And then I was able to narrow down the test to the following (note: this doesn't even use torch.compile) -- https://gist.github.com/angelayi/e6ea40b88d289f6c16e5ea823e70cfb9. From this test, I was able to make the test pass by adding a [warmup](https://gist.github.com/angelayi/e6ea40b88d289f6c16e5ea
- **Note:** The reason of not using all FlashInfer Blockscale FP8 kernels for linear layer is we're seeing accuracy drop from 95% to 88% when running GSM8K. There's an internal logic in CPP that uses non-swapAB kernel for M>=32 gemms. Therefore an alternative way is only use swapAB in M<32 scenario, l
...
- - **[Should probably implement&merge it before this PR]** Refactor out the existing normalizations in config/[model.py](http://model.py/)  so it can be shared by model-arch-parser and hf_config during the migration
