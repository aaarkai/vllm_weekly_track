# Weekly Release Report for vllm-project/vllm (2025-12-05)

This week merged 229 PRs from 125 contributors. Key areas: features 8, fixes 10, performance 11.

## Executive Summary

本次发布在核心功能与性能方面有重要更新。Features 方面，为 Marlin 内核增加了 w4a8 量化支持，并扩展了对 Qwen3-Omni-MoE 模型 BitsAndBytes 量化的支持。Model 支持新增了 Mistral Large 3 和 Ministral 3，并引入了 DeepSeek-V3.2 的工具解析器。

在 Fixes 与 Performance 方面，修复了 LoRA 模块注册、调度策略及解码过程中的多个关键问题。性能优化包括为 CUDA 启用独立的 shared_experts 流以避免页面锁定内存传输。此外，本版本包含多项 Breaking Changes：删除了 HF 版本的 Phi 4 MM 模型，弃用了 `merge_by_field_config` 参数，并重构了部分内部接口。升级时需注意相关改动并参考更新后的文档。
...
* Fix boolean nested params, add dict format support, and enhance plotting for vllm bench sweep ([#29025](https://github.com/vllm-project/vllm/pull/29025)) by @copilot-swe-agent
...
* Bump actions/setup-python from 6.0.0 to 6.1.0 ([#29768](https://github.com/vllm-project/vllm/pull/29768)) by @dependabot
...
- Note: This is a part of my attempt to improve GGUF file support and more specifically, my attempt to restore Qwen3 MoE + GGUF support (my WIP branch currently suffers from gibberish outputs on Qwen3 MoE GGUF models and encountered this bug while testing which part of GGUF handling is broken; that wa
