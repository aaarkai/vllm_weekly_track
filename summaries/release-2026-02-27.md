# Weekly Release Report for vllm-project/vllm (2026-02-27)

This week merged 235 PRs from 132 contributors. Key areas: features 5, fixes 50, performance 30.

## Executive Summary

本周发布主要围绕模型推理性能优化、系统稳定性提升与测试基础设施强化展开。核心特性包括为 Nemotron-H MTP 及 Mamba 模型新增推测解码支持，并引入了 TRTLLM DSV3 Router GEMM 内核，在特定场景下实现了约 6% 的速度提升。同时，本次更新支持了量化后的 LoRA 适配器，并进行了 Harmony 与 MCP 响应测试的大规模重构，以提高测试的确定性和稳定性。

在缺陷修复方面，解决了涉及 Qwen、Mamba 等多个模型系列的量化加载、缓存、推理解析及工具调用等相关问题，并针对 AMD MI300X、CUDA 12.8 等特定硬件与驱动环境进行了关键修复。性能优化工作持续进行，涵盖 KV 缓存更新、池化模型计算、MoE 内核配置及基准测试工具等多个层面的改进。本次更新未包含重大突破性变更，但建议用户在升级后关注特定模型（如 Qwen3.5、MLA）的量化配置与硬件兼容性。
...
- * Note: I think this kernel can be further optimized. For example we can add one more dimension `triton.cdiv(vocab_size, BLOCK_SIZE)` to the grid and use atomic operations, in order to parallelize. But this kernel is not launched many times, so I didn't try those optimizations.
- A more likely upgrade path is to Zensical, which is more actively maintained by the creators of MkDocs Material.
- From local testing, it looks like it is just barely exceeding the timeout due to AITER JIT compilation taking 130 seconds for one particular kernel (it took 250 seconds for the server to come up overall). I loosened the overall timeout restriction by a couple minutes, increasing it from 240 to 360 s
...
- Once upon a time, in a bustling city, there was a talented developer named Alex. With his sharp mind and boundless creativity, Alex was known for his innovative projects and groundbreaking ideas. One day, while working on a complex software application, Alex stumbled upon a peculiar issue that requi
