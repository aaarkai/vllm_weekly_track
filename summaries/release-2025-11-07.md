# Weekly Release Report for vllm-project/vllm (2025-11-07)

This week merged 170 PRs from 112 contributors. Key areas: features 6, fixes 7, performance 18.

## Executive Summary

本周发布聚焦于性能优化与模型支持扩展。关键改进包括对MoE和LoRA内核的精细化调优，如fused_moe_lora配置分离加载和早期退出机制，以及TP与EP共享专家重叠带来的端到端性能提升。新功能涵盖llama 4缩放支持、kimi reasoning解析器集成，并增强了多模态模型（如Qwen Omni）和硬件后端（如ROCm AITER MHA）的兼容性。此外，异步调度与结构化输出的协同工作得到进一步完善。

在修复方面，解决了采样token ID预分配大小错误、混合惩罚批次处理异常及多个模型加载问题（如Qwen3-Reranker-8B）。破坏性变更包括移除已弃用参数（如rope-scaling）并升级依赖项（llguidance至1.3.0、torchvision至v0.23.0）。升级时需注意Executor collective_rpc接口返回类型变更，并确保测试环境适配模型路径调整。
...
- Upgrade `llguidance` to latest version 1.3.0. It contains several bug fixes since the current version (v0.7.30).
- As indicated in the change logs, there's no breaking change from v0.7 -> v1.0. It was just an indication of a stable release.
...
- Note:
- Add Unit Tests for the migration of LMCache adapter into vLLM codebase, following up: https://github.com/vllm-project/vllm/pull/25542. These Unit tests primarily test the interface and not the logic.
- Note: This PR also relies on #27698 to land first. Graph partition does NOT compose with aot_compile yet.
- Note: Based on PR #21229 and #26319
